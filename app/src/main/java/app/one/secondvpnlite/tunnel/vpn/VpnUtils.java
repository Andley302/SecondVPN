package app.one.secondvpnlite.tunnel.vpn;
...
public class VpnUtils {
    ...
    public static void killProcess(String pidString, String signal) throws Exception {
        ...
        for (int i = 0; i < cmds.length;i++) {
            try {
                ...
                ////AppLogManager.logDebug(String.format("error killing process: %s, %s", pidString, ioe.getMessage()));
            }
        }
    }
	...
	private final static String DEFAULT_PRIMARY_DNS_SERVER = "8.8.8.8";
    private final static String DEFAULT_SECONDARY_DNS_SERVER = "8.8.4.4";
    ...
    private static List<String> getDNS(Context context){
        List<String> dnsResolver = new ArrayList<String>();
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                ...
                final String NetworkDNS = dnslist.replace("[","").replace("]","").replace("/","").replace(" ","");
                List<String> dns_list = new ArrayList<String>(Arrays.asList(NetworkDNS.split(",")));
                dnsResolver.add(dns_list.get(0));
